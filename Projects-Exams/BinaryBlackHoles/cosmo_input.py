...
cb_class = ['BNS']# list of compact objects type 
...
if metmod_type == 'linear':
	...
	print("Please choose between the following models: linear")
	exit()
...
met_uncertainty = 'Yes' # if 'Yes' the uncertainty due to metallicity will be computed
met_slope_sprd = 0.14 # from De Cia et al. 2018
...
N_iter = 1000 #number of iteration to be done for evaluating the uncertainty: it must be at least one!
...
N_cat = 100 # number of CB per redshift bin
...
path_out = sim+'/output/'#path to the output folder. The output are divided by CB class
